...
uint32_t write(int __fd, const uint8_t *buffer, size_t __n) {
  *I2C_MASK = 0b0011;

  uint32_t data = 0;
  for (int i = 1; i < __n; i++) {
    data = 0;
    data |= (buffer[0] << 8) | buffer[i];
    *I2C_DATA = data;
  }
...
// TODO: ultra dirty. Clean this up. Implementation is found in wspr.c
...
int oledInit(int iChannel, int iAddr, int iType, int bFlip, int bInvert) {
  ...
  file_i2c = 1; // Simulate opening the file
  *I2C_DEVICE_ADDR = 60;
